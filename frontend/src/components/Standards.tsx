...
//             {selectedTab
//               ? `This is a description for ${selectedTab.label}. Add more content or details here to make it engaging.`
...
const Standards = () => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-8 border border-tg-100 rounded-xl bg-white">
      {/* Navigation */}
      <nav className="mb-12">
        <ul className="flex justify-evenly">
          {tabs.map((item) => (
        ...
        <div className="flex-1 text-left max-w-lg">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: selectedTab ? selectedTab.description : "Select a tab to see details.",
            }}
          />
        </div>
        ...
        <div className="flex-1 flex justify-center items-center">
          <AnimatePresence mode="wait">
            <motion.img
              ...
              exit={{ scale: 0.9, opacity: 0 }}
              transition={{ duration: 0.3 }}
              className="w-94 h-94 object-cover rounded-lg shadow-xl"
            />
          </AnimatePresence>
        </div>
      </main>
